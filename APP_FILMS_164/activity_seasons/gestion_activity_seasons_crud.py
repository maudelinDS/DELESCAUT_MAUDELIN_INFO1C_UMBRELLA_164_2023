...
@app.route("/activity_seasons_afficher/<int:id_film_sel>", methods=['GET', 'POST'])
def activity_seasons_afficher(id_film_sel):
    print(" films_genres_afficher id_film_sel ", id_film_sel)
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT t_season.season_id, t_season.season_name, GROUP_CONCAT(t_activity.name_activity) AS ActivitySeason
FROM t_activity_season
RIGHT JOIN t_season ON t_season.season_id = t_activity_season.fk_season
LEFT JOIN t_activity ON t_activity.activity_id = t_activity_season.fk_activity
GROUP BY t_season.season_id,t_season.season_name
"""
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING season_id= %(value_id_film_selected)s"""
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    flash("""La table "t_season" est vide. !""", "warning")
                elif not data_genres_films_afficher and id_film_sel > 0:
                    # Si l'utilisateur change l'season_id dans l'URL et qu'il ne correspond à aucun film
            ...
            raise ExceptionFilmsGenresAfficher(f"fichier : {Path(__file__).name}  ;  {activity_seasons_afficher.__name__} ;"
                                               f"{Exception_films_genres_afficher}")

    print("films_genres_afficher  ", data_genres_films_afficher)
    # Envoie la page "HTML" au serveur.
    return render_template("activity_seasons/activity_seasons_afficher.html", data=data_genres_films_afficher)
...
@app.route("/edit_activity_season_selected", methods=['GET', 'POST'])
def edit_activity_season_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT season_id, season_name FROM t_season ORDER BY season_id ASC"""
                mc_afficher.execute(strsql_genres_afficher)
            data_genres_all = mc_afficher.fetchall()
            print("dans edit_genre_film_selected ---> data_genres_all", data_genres_all)

            # Récupère la valeur de "season_id" du formulaire html "films_genres_afficher.html"
            # l'utilisateur clique sur le bouton "Modifier" et on récupère la valeur de "season_id"
            # grâce à la variable "id_film_genres_edit_html" dans le fichier "films_genres_afficher.html"
            # href="{{ url_for('edit_genre_film_selected', id_film_genres_edit_html=row.season_id) }}"
            ...
            data_genre_film_selected, data_genres_films_non_attribues, data_genres_films_attribues = \
                activity_environs_afficher(valeur_id_film_selected_dictionnaire)

            print(data_genre_film_selected)
            lst_data_film_selected = [item['season_id'] for item in data_genre_film_selected]
            print("lst_data_film_selected  ", lst_data_film_selected,
                  type(lst_data_film_selected))

            # Dans le composant "tags-selector-tagselect" on doit connaître
            # les genres qui ne sont pas encore sélectionnés.Erreur : fichier : gestion_films_genres_crud.py ; edit_genre_film_selected ; fichier : gestion_films_genres_crud.py ; genres_films_afficher_data ; <class 'pymysql.err.OperationalError'> exception dans DBconnection methode exit : (1140, "In aggregated query without GROUP BY, expression #2 of SELECT list contains nonaggregated column 'DELESCAUT_MAUDELIN_INFO1C_UMBRELLA_164_2023.t_activity_season.score'; this is incompatible with sql_mode=only_full_group_by") fichier : gestion_films_genres_crud.py ; edit_genre_film_selected ; fichier : gestion_films_genres_crud.py ; genres_films_afficher_data ; <class 'pymysql.err.OperationalError'> exception dans DBconnection methode exit : (1140, "In aggregated query without GROUP BY, expression #2 of SELECT list contains nonaggregated column 'DELESCAUT_MAUDELIN_INFO1C_UMBRELLA_164_2023.t_activity_season.score'; this is incompatible with sql_mode=only_full_group_by") <class 'APP_FILMS_164.erreurs.exceptions.ExceptionEditGenreFilmSelected'> ￼×
            ...
            lst_data_genres_films_old_attribues = [item['activity_id'] for item in data_genres_films_attribues]
            ...
            raise ExceptionEditGenreFilmSelected(f"fichier : {Path(__file__).name}  ;  "
                                                 f"{edit_activity_season_selected.__name__} ; "
...
@app.route("/update_activity_season_selected", methods=['GET', 'POST'])
def update_activity_season_selected():
    if request.method == "POST":
        try:
            ...
            # OM 2021.05.02 Une liste de "activity_id" qui doivent être effacés de la table intermédiaire "t_activity_season".
            ...
            # Une liste de "activity_id" qui doivent être ajoutés à la "t_activity_season"
            ...
            # "fk_film"/"season_id" et "fk_genre"/"activity_id" dans la "t_activity_season"
            strsql_insert_genre_film = """INSERT INTO t_activity_season (activity_environ_id, fk_activity, fk_season)
                                                    VALUES (NULL, %(value_fk_genre)s, %(value_fk_film)s)"""

            # SQL pour effacer une (des) association(s) existantes entre "season_id" et "activity_id" dans la "t_activity_season"
            strsql_delete_genre_film = """DELETE FROM t_activity_season WHERE fk_activity = %(value_fk_genre)s AND fk_season = %(value_fk_film)s"""

            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des genres à INSÉRER dans la "t_activity_season".
                ...
                # Pour le film sélectionné, parcourir la liste des genres à EFFACER dans la "t_activity_season".
                # Si la liste est vide, la boucle n'est pas parcourue.
            ...
            raise ExceptionUpdateGenreFilmSelected(f"fichier : {Path(__file__).name}  ;  "
                                                   f"{update_activity_season_selected.__name__} ; "
                                                   f"{Exception_update_genre_film_selected}")

    # Après cette mise à jour de la table intermédiaire "t_activity_season",
...
def activity_environs_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        strsql_film_selected = """SELECT season_id, season_name,  GROUP_CONCAT(activity_id) as ActivitySeason
                          FROM t_activity_season
                          INNER JOIN t_season ON t_season.season_id = t_activity_season.fk_season
                          INNER JOIN t_activity ON t_activity.activity_id = t_activity_season.fk_activity
                          WHERE season_id = %(value_id_film_selected)s
                          GROUP BY season_id, season_name"""

        strsql_genres_films_non_attribues = """SELECT activity_id, name_activity FROM t_activity WHERE activity_id not in(SELECT activity_id as idActivitySeason FROM t_activity_season
                                                    INNER JOIN t_season ON t_season.season_id = t_activity_season.fk_season
                                                    INNER JOIN t_activity ON t_activity.activity_id = t_activity_season.fk_activity
                                                    WHERE season_id = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT season_id, activity_id, name_activity FROM t_activity_season
                                            INNER JOIN t_season ON t_season.season_id = t_activity_season.fk_season
                                            INNER JOIN t_activity ON t_activity.activity_id = t_activity_season.fk_activity
                                            WHERE season_id = %(value_id_film_selected)s"""
