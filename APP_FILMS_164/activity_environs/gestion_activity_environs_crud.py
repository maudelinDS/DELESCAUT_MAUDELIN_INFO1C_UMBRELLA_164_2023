...
GROUP BY t_environ.environ_id,t_environ.environ_name
"""
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING environ_id= %(value_id_film_selected)s"""
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    flash("""La table "t_environ" est vide. !""", "warning")
                elif not data_genres_films_afficher and id_film_sel > 0:
                    # Si l'utilisateur change l'environ_id dans l'URL et qu'il ne correspond à aucun film
...
def edit_activity_environ_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT environ_id, environ_name FROM t_environ ORDER BY environ_id ASC"""
                mc_afficher.execute(strsql_genres_afficher)
            data_genres_all = mc_afficher.fetchall()
            print("dans edit_genre_film_selected ---> data_genres_all", data_genres_all)

            # Récupère la valeur de "environ_id" du formulaire html "films_genres_afficher.html"
            # l'utilisateur clique sur le bouton "Modifier" et on récupère la valeur de "environ_id"
            # grâce à la variable "id_film_genres_edit_html" dans le fichier "films_genres_afficher.html"
            # href="{{ url_for('edit_genre_film_selected', id_film_genres_edit_html=row.environ_id) }}"
            ...
            # les genres qui ne sont pas encore sélectionnés.Erreur : fichier : gestion_films_genres_crud.py ; edit_genre_film_selected ; fichier : gestion_films_genres_crud.py ; genres_films_afficher_data ; <class 'pymysql.err.OperationalError'> exception dans DBconnection methode exit : (1140, "In aggregated query without GROUP BY, expression #2 of SELECT list contains nonaggregated column 'DELESCAUT_MAUDELIN_INFO1C_UMBRELLA_164_2023.t_activity_environ.score'; this is incompatible with sql_mode=only_full_group_by") fichier : gestion_films_genres_crud.py ; edit_genre_film_selected ; fichier : gestion_films_genres_crud.py ; genres_films_afficher_data ; <class 'pymysql.err.OperationalError'> exception dans DBconnection methode exit : (1140, "In aggregated query without GROUP BY, expression #2 of SELECT list contains nonaggregated column 'DELESCAUT_MAUDELIN_INFO1C_UMBRELLA_164_2023.t_activity_environ.score'; this is incompatible with sql_mode=only_full_group_by") <class 'APP_FILMS_164.erreurs.exceptions.ExceptionEditGenreFilmSelected'> ￼×
...
def update_activity_environ_selected():
    if request.method == "POST":
        try:
            ...
            # OM 2021.05.02 Une liste de "activity_id" qui doivent être effacés de la table intermédiaire "t_activity_environ".
            ...
            # Une liste de "activity_id" qui doivent être ajoutés à la "t_activity_environ"
            ...
            # "fk_film"/"environ_id" et "fk_genre"/"activity_id" dans la "t_activity_environ"
            ...
            # SQL pour effacer une (des) association(s) existantes entre "environ_id" et "activity_id" dans la "t_activity_environ"
            ...
            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des genres à INSÉRER dans la "t_activity_environ".
                ...
                # Pour le film sélectionné, parcourir la liste des genres à EFFACER dans la "t_activity_environ".
                # Si la liste est vide, la boucle n'est pas parcourue.
            ...
            raise ExceptionUpdateGenreFilmSelected(f"fichier : {Path(__file__).name}  ;  "
                                                   f"{update_activity_environ_selected.__name__} ; "
                                                   f"{Exception_update_genre_film_selected}")

    # Après cette mise à jour de la table intermédiaire "t_activity_environ",
...
def activity_environs_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        strsql_film_selected = """SELECT environ_id, environ_name,  GROUP_CONCAT(activity_id) as ActivityEnviron
                          ...
                          GROUP BY environ_id, environ_name"""

        strsql_genres_films_non_attribues = """SELECT activity_id, name_activity FROM t_activity WHERE activity_id not in(SELECT activity_id as idActivityEnviron FROM t_activity_environ
                                                    INNER JOIN t_environ ON t_environ.environ_id = t_activity_environ.fk_environ
                                                    INNER JOIN t_activity ON t_activity.activity_id = t_activity_environ.fk_activity
                                                    WHERE environ_id = %(value_id_film_selected)s)"""
