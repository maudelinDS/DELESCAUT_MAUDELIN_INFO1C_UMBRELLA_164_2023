...
def film_update_wtf():
    # L'utilisateur vient de cliquer sur le bouton "EDIT". Récupère la valeur de "weather_id"
...
def film_delete_wtf():
    ...
    btn_submit_del = None
    # L'utilisateur vient de cliquer sur le bouton "DELETE". Récupère la valeur de "weather_id"
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_activity_weather WHERE fk_weather = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_weather WHERE weather_id = %(value_id_film)s"""
            # Manière brutale d'effacer d'abord la "fk_film", même si elle n'existe pas dans la "t_activity_weather"
            # Ensuite on peut effacer le film vu qu'il n'est plus "lié" (INNODB) dans la "t_activity_weather"
            with DBconnection() as mconn_bd:
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_weather WHERE weather_id = %(value_id_film)s"""
